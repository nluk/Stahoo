...
class AddEditOperationFragment : BaseFragment(), DatePickerDialog.OnDateSetListener,
    ...
    private var operation = MainOperation()

    private val subOperations: MutableList<SubOperation> = ArrayList()
    private val subOperationAdapter = SubOperationAdapter(
        subOperations,
    ...
    private fun setupFields() {
        if (editMode) {
            operation_name.setText(operation.name)
        }
    ...
    private fun getEvent() {
        if (arguments?.get("operationId") != null) {
            editMode = true
            operation = operationRepository.findById(arguments!!.getInt("operationId"))
        }
    }
    ...
    private fun setAdapters() {
        ...
        val categoriesAdapter =
            ArrayAdapter(context, android.R.layout.simple_spinner_item, resources.getStringArray(R.array.categories))
        ...
        val cycleTypeAdapter = ArrayAdapter(
            context,
            android.R.layout.simple_spinner_item,
            resources.getStringArray(R.array.operation_cyclic)
        )
        categoriesAdapter.setDropDownViewResource(android.R.layout.simple_dropdown_item_1line)
    ...
    private fun setListeners() {
        ...
        add_split.setOnClickListener {
            AddSplitDialog(context!!) { newSplit ->
                subOperations.add(newSplit)
